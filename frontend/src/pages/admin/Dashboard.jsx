...
import { getOrders, updateOrderStatus, getTodayStats } from '../../services/api';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => { if (authenticated) fetchData(); }, 30000);
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '11px' }}>Painel Administrativo</p>
          </div>
        </div>
        <button
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              borderBottom: activeMenu === item.key ? '3px solid var(--purple-600)' : '3px solid transparent',
              color: activeMenu === item.key ? 'var(--purple-700)' : 'var(--gray-500)',
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', overflowX: 'auto', paddingBottom: '4px' }}>
            {[{ value: '', label: 'Todos' }, ...Object.entries(STATUS_CONFIG).map(([k, v]) => ({ value: k, label: `${v.icon} ${v.label}` }))].map(f => (
              <button
                ...
                style={{
                  padding: '7px 14px', borderRadius: '99px', border: '2px solid',
                  ...
                  fontWeight: '700', fontSize: '12px', whiteSpace: 'nowrap', cursor: 'pointer'
                }}
              >{f.label}</button>
            ))}
          </div>
