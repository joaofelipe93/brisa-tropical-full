...
export const adminController = {
  ...
  listProducts(req, res) {
    res.json(adminRepository.findAllProductsAdmin());
  },
  ...
  listCategories(req, res) {
    res.json(adminRepository.findAllCategoriesAdmin());
  },
  ...
  listSteps(req, res) {
    res.json(adminRepository.findAllSteps());
  },

  updateStep(req, res) {
    const { title, subtitle, emoji, min_selections, max_selections, sort_order, active } = req.body;
    if (!title || !subtitle)
      return res.status(400).json({ error: 'title e subtitle são obrigatórios' });
    const step = adminRepository.findStepById(req.params.id);
    if (!step) return res.status(404).json({ error: 'Passo não encontrado' });
    adminRepository.updateStep(req.params.id, {
      title, subtitle, emoji: emoji || step.emoji,
      minSelections: min_selections ?? step.min_selections,
      maxSelections: max_selections ?? step.max_selections,
      sortOrder: sort_order ?? step.sort_order,
  ...
  createOption(req, res) {
    ...
    const id = adminRepository.createOption({
      stepId: step_id, name,
      extraPrice: extra_price || 0,
      sortOrder: sort_order || 0,
    });
    res.status(201).json({ success: true, id });
  },
  ...
  updateOption(req, res) {
    const { name, extra_price, sort_order, active } = req.body;
    if (!name) return res.status(400).json({ error: 'name é obrigatório' });
    const option = adminRepository.findOptionById(req.params.option_id);
    if (!option) return res.status(404).json({ error: 'Opção não encontrada' });
    adminRepository.updateOption(req.params.option_id, {
      name,
      extraPrice: extra_price || 0,
      sortOrder: sort_order || 0,
      active: active !== undefined ? active : option.active,
    });
    res.json({ success: true });
  },
  ...
  deleteOption(req, res) {
    const option = adminRepository.findOptionById(req.params.option_id);
    if (!option) return res.status(404).json({ error: 'Opção não encontrada' });
    adminRepository.deleteOption(req.params.option_id);
    res.json({ success: true });
  },
};
