...
export const orderController = {
  ...
  async createOrder(req, res) {
    ...
    const createOrderTransaction = db.transaction(() => {
      ...
      // Buscar bairro
      const neighborhood = productRepository.findNeighborhoodById(neighborhood_id);
      ...
      return { orderId, cust, neighborhood, enrichedItems, subtotal, total, deliveryFee: neighborhood.delivery_fee };
    });

    try {
      const { orderId, cust, neighborhood, enrichedItems, total, deliveryFee } = createOrderTransaction();

      const order    = orderRepository.findOrderById(orderId);
      const settings = Object.fromEntries(orderRepository.findAllSettings().map(s => [s.key, s.value]));
      ...
      const waLinks = getWhatsAppLinks({ order, customer: cust, items: itemsForWA, neighborhood, settings });
